...
export async function getAssetOverview(
  ...
  if (!isTauriRuntime()) {
    return buildPreviewAssetOverview(asset, category)
  }
...
function buildPreviewAssetOverview(asset: MarketAsset, category?: string): AssetOverviewResponse {
  const seeds = previewAssetSeeds[asset]
  const filteredSeeds =
    category && category !== "all" ? seeds.filter((seed) => seed.category_id === category) : seeds
  const rows = filteredSeeds.map((seed, index) => buildPreviewAssetRow(asset, seed, index))
  const categories = marketAssetConfigs[asset].categories.map((category) => ({
    id: category.id,
    label_key: category.i18nKey,
    total: seeds.filter((row) => row.category_id === category.id).length,
  }))
